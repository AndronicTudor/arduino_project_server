...
class ArduinoApiView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            if data.get('update'):
                query = SensorData.objects.filter(ard_id=data.get('ard_id')).order_by('-created').first()
                query.lat = data.get('lat')
                query.lon = data.get('lon')
                query.save()
                return Response({"detail": "Update succesful"}, status=status.HTTP_200_OK)
            else:
                serializer.save()
...
class ChartData(APIView):
    ...
    def get(self, request,  format=None):
        labels = ['Temperature max', 'Temperature min', 'Humidity max', 'Humidity min', 'Light max', 'Light min']
        ...
        sensor_data = [max(temp), min(temp), max(hum), min(hum), max(lig), min(lig)]
